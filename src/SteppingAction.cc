...
if (thePostPoint->GetStepStatus() == fGeomBoundary &&
      thePostPV->GetName()=="World" &&
      thePrePV->GetName()=="Target")
  {
   ...
   if (idTuple != 0){
    ...
    G4double energy = thePostPoint->GetKineticEnergy();
    G4ThreeVector position = thePostPoint->GetPosition();
    G4ThreeVector preMomentum = thePostPoint->GetMomentum(); 
    
     G4AnalysisManager::Instance()->FillNtupleDColumn(idTuple, 0, position.y()/mm);
     G4AnalysisManager::Instance()->FillNtupleDColumn(idTuple, 1, position.z()/mm);
     G4AnalysisManager::Instance()->FillNtupleDColumn(idTuple, 2, preMomentum.x()/MeV);
     G4AnalysisManager::Instance()->FillNtupleDColumn(idTuple, 3, preMomentum.y()/MeV);
     G4AnalysisManager::Instance()->FillNtupleDColumn(idTuple, 4, preMomentum.z()/MeV);
     G4AnalysisManager::Instance()->FillNtupleDColumn(idTuple, 5, energy/MeV);
     G4AnalysisManager::Instance()->FillNtupleDColumn(idTuple, 6, 0);
