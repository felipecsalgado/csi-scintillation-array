  ...
  fPhysiLayer(nullptr)
{
  ...
  fNbOfAbsor = 2;
  for(G4int i=1; i<=fNbOfAbsor; i = i + 2) { 
    fAbsorThickness[i] = 200*um;
    SetAbsorMaterial(i,"TiO2");
    
    G4int j = i + 1;
    fAbsorThickness[j] = 2*mm;
    SetAbsorMaterial(j,"CsI");
  }  

  fNbOfLayers        = 150;
  separator_thickness = 200.*um;
  separator_material = G4NistManager::Instance()->FindOrBuildMaterial("TiO2");
  nStacks = 51.;
...
void DetectorConstruction::DefineMaterials()
{
  ...
  //fWorldTarget = G4NistManager::Instance()->FindOrBuildMaterial();
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  fSolidWorld = new G4Box("World",                                //its name
                   fWorldSizeX/2 + 0.5*m,fWorldSizeY/2,fWorldSizeZ/2);  //its size
  ...
  fSolidCalor = new G4Box("Calorimeter",                                  
                       fCalorThickness/2,fCalorSizeY/2 + separator_thickness/2,fCalorSizeZ/2);
  ...
  // Copy and place different calorimeters on top of each other
  // to build the stack of scintillation crystals
  for (G4int k=0; k<nStacks; ++k) {
       G4double half_heigth_cal = fCalorSizeY/2 + separator_thickness/2 ;
       G4double ypos_init = -(nStacks/2. * half_heigth_cal * 2) + half_heigth_cal;
       G4double ypos = ypos_init + half_heigth_cal * 2 * k;
  ...
  fLogicLayer->SetVisAttributes(G4Color(1., 1., 0., 0.05));
  ...
  for (G4int k=1; k<=fNbOfAbsor; ++k) {
    ...
    fLogicAbsor[k]->SetVisAttributes(G4Color(1., 0., 0., 0.05));
        
    
    //
    // Separator
    // Place the separators on top and bottom of the layer
    //
    fSolidSep = new G4Box("Separator",                //its name
                          fAbsorThickness[k]/2, (separator_thickness/2)/2,fCalorSizeZ/2);

    fLogicSep = new G4LogicalVolume(fSolidSep,    //its solid
                                    separator_material, //its material
                                    "SeparatorTiO2");
    fLogicSep->SetVisAttributes(G4Color(0., 1., 0., 0.05));
    
    // Top
    fPhysiSep = new G4PVPlacement(0,              
                         G4ThreeVector(xcenter,fCalorSizeY/2 + (separator_thickness/2)/2,0.),
                         fLogicSep,               
                         "SeparatorPhys",
                         fLogicLayer,                  
                         false,                      
  ...
  // Target
  //
  fSolidTarget = new G4Box("target",                                  
                       500.*um/2, 25.0*mm/2, 25.0*mm/2);
  ...
  fPhysiTarget = new G4PVPlacement(0,                     //no rotation
                                 G4ThreeVector(-60.*cm, 0., 0.),        //at (0,0,0)
  ...
  // Magnet
  //
  fMagnetGapSolid = new G4Box("Magnet50cm", //its name
                                 20.*cm / 2, 20. * mm / 2, 20 * mm / 2); //mesh->TessellatedMesh();
  fMagnetGapLogical = new G4LogicalVolume(fMagnetGapSolid,
                                          fWorldMaterial,
                                          "MagnetGapLogical");
  fMagnetGapPhys =   new G4PVPlacement(0,
                                       G4ThreeVector(-40.*cm, 0., 0.),
                                       fMagnetGapLogical,
                                       "MagnetGapPhysical",
                                       fLogicWorld,
                                       false,
                                       0);
  //fMagnetGapLogical->SetVisAttributes(G4Color(1., 1., 1., 1));

  // Definition of the magnetic field
  G4ThreeVector magnet_field = G4ThreeVector(0.0, 3.20 * tesla, 0.0);
  //
  //Magnet configuration
  // set step limit in box with magnetic field
  G4UserLimits *userLimits = new G4UserLimits(100 * um);
  fMagnetGapLogical->SetUserLimits(userLimits);
  // magnetic field
  // fMagneticField = new LXeMagneticField();
  G4MagneticField *magField = new G4UniformMagField(magnet_field);
  fFieldMgr = new G4FieldManager();
  fFieldMgr->SetDetectorField(magField);
  fFieldMgr->CreateChordFinder(magField);
  fFieldMgr->GetChordFinder()->SetDeltaChord(75 * um);

  G4bool forceToAllDaughters = true;
  fMagnetGapLogical->SetFieldManager(fFieldMgr, forceToAllDaughters);
